#!/usr/bin/env python3
"""
extract_crocov2_features.py

Extract raw ViT encoder features from a CroCo v2 checkpoint, loaded via the
AsymmetricMASt3R architecture (MASt3R/DUSt3R are both fine-tuned from a
CroCo v2 encoder-decoder, so the same loader/model class works here) — the
feat1/feat2 tensors that sit between the patch embedding and the
cross-attention decoder. No pooling or dimensional reduction is applied.
...
Usage
─────
python extract_crocov2_features.py \\
    --ann_json   data/annotations.json \\
    --img_dir    data/images \\
    --weights    checkpoints/CroCo_V2_ViTLarge_BaseDecoder.pth \\
    --output_dir features/crocov2_enc/
"""
...
# ── CroCo v2 / MASt3R imports (AsymmetricMASt3R loads the CroCo v2 encoder) ──
# Assumes the repo is on the path, either installed or via sys.path.append.
import mast3r.utils.path_to_dust3r  # noqa — sets up dust3r on sys.path
...
def get_args():
    p = argparse.ArgumentParser(
        description="Extract raw ViT encoder features from a CroCo v2 checkpoint (loaded via AsymmetricMASt3R).",
    ...
    p.add_argument("--weights",
                   default="checkpoints/CroCo_V2_ViTLarge_BaseDecoder.pth",  # TODO: set to your local CroCo v2 checkpoint path
                   help="Local .pth path (or HuggingFace repo id, if you have one that "
                        "AsymmetricMASt3R.from_pretrained() can load) for the CroCo v2 "
                        "checkpoint. The official release is CroCo_V2_ViTLarge_BaseDecoder.pth "
                        "from https://github.com/naver/croco — no HF Hub id for it was found, "
                        "so this defaults to a local path placeholder rather than guessing one.")
    p.add_argument("--image_size", type=int, default=224,
                   help="Longest-side target for load_images() resizing. "
...
# ─────────────────────────────────────────────────────────────────────────────
# 2.  Annotation loaders  (unchanged from sam3d script)
# ─────────────────────────────────────────────────────────────────────────────
...
def load_one(img_path: str, image_size: int, patch_size: int) -> dict:
    ...
    # MASt3R requires true_shape for positional embeddings
...
def main():
    ...
    # ── Model ─────────────────────────────────────────────────────────────────
    # Load as AsymmetricMASt3R regardless of whether the checkpoint is actually
    # MASt3R or DUSt3R — the encoder is identical in both; from_pretrained()
    # will raise if the architecture truly doesn't match.
    ...
    for sample in tqdm(samples, desc="Extracting"):
        ...
        # Load + preprocess using the exact same transform as MASt3R inference
        try:
