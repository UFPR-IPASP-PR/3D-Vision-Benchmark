#!/usr/bin/env python3
"""
extract_da2_features.py
─────────────────────────
Feature extraction from DINOv2 or DepthAnything V2 (DPT-DINOv2) encoders.
...
────────────────────────
  ...
  --model depth_anything_v2
    Loads DepthAnything V2 (DepthAnythingV2) from a local checkpoint.
    ...
    to the DINOv2 encoder embedded inside DepthAnythingV2.
    ...
    The token dimension D and patch grid P depend on the chosen encoder size:
      ...
      vitg → D=1536, patch grid ~37×37

Usage examples
──────────────
# DINOv2 (vitl, default)
python extract_da2_features.py \\
    --model       dinov2 \\
    --ann_json    data/annotations.json \\
    --img_dir     data/images \\
    --output_dir  features/dinov2/

# DepthAnything V2 vitb encoder
python extract_da2_features.py \\
    --model          depth_anything_v2 \\
    --encoder        vitb \\
...
def get_args():
    p = argparse.ArgumentParser(
        description="Extract DINOv2 or DepthAnything V2 encoder tokens.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    # ── what to extract ────────────────────────────────────────────────────
    p.add_argument(
        "--model", required=True, choices=["dinov2", "depth_anything_v2"],
        help="Which encoder to load and extract from.",
    )
    p.add_argument(
        "--encoder", default="vitl", choices=["vits", "vitb", "vitl", "vitg"],
        help=(
            "ViT backbone size. 'vitg' is only valid for --model depth_anything_v2."
        ),
    )
    ...
    p.add_argument(
        ...
        help=(
            ...
            "Required when --model depth_anything_v2 is used. "
            "Unlike V1, DepthAnything V2 has no HuggingFace Hub pretrained loader."
        ),
    )
    p.add_argument(
        ...
        help=(
            ...
            "multiple of 14). Matches the default used in the official V2 repo."
        ),
    )
...
def build_depth_anything_v2(
    ...
    Unlike V1, DepthAnythingV2 has no HuggingFace from_pretrained() loader
    and no localhub flag, so --da_checkpoint is mandatory here.
    """
    if checkpoint is None:
        raise ValueError(
            "--da_checkpoint is required for --model depth_anything_v2. "
            "DepthAnything V2 does not have a HuggingFace Hub pretrained loader."
        )
...
def preprocess_for_depth_anything_v2(
    ...
    Delegates to model.image2tensor(), which is the canonical preprocessing
    method in the V2 codebase (defined in DepthAnythingV2.image2tensor).
    It applies the same Resize + NormalizeImage + PrepareForNet pipeline
    used in run.py and app.py.

    image2tensor expects a BGR numpy array (uint8), matching cv2.imread output.
    It internally calls cv2.cvtColor(raw_image, cv2.COLOR_BGR2RGB).

    Returns: (1, 3, H', W') float32 tensor on `device`, where H' and W' are
    multiples of 14 with the short side >= input_size.
    """
    raw = cv2.imread(img_path)
    if raw is None:
        raise FileNotFoundError(f"cv2.imread failed: {img_path}")

    # image2tensor returns (tensor, (orig_h, orig_w)); we only need the tensor.
    tensor, _ = model.image2tensor(raw, input_size)
    # image2tensor already places the tensor on the device detected internally,
    # but we move it explicitly to the user's requested device for consistency.
...
def extract_depth_anything_v2_tokens(
    ...
    """
    Run DepthAnythingV2's pretrained DINOv2 backbone (model.pretrained) and
    ...
    model.pretrained is a DinoVisionTransformer (depth_anything_v2/dinov2.py),
    which exposes the same get_intermediate_layers() API as the torch.hub
    DINOv2 models. n=1 selects the last transformer block's output.

    The V2 DinoVisionTransformer may contain register tokens
    (num_register_tokens > 0); get_intermediate_layers() already strips them
    from the patch token sequence before returning, so no extra handling is
    needed here.
    """
    # model.pretrained is the DinoVisionTransformer inside DepthAnythingV2.
