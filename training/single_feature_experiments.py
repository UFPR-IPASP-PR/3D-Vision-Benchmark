#!/usr/bin/env python3
"""
Vehicle attribute classifier trained on DINOv3 features extracted by
extraction/extract_hf.py.

extraction/extract_hf.py saves ALL images from one extraction run into a
single compressed .npz archive with two arrays: 'features' (shape [N, D])
and 'filenames' (shape [N], basenames with extension). This script loads
that single archive, matches rows to annotated images by exact filename,
and trains multi-head classifiers on top of those flat per-image vectors.

Usage:
    python training/single_feature_experiments.py \
...
def get_args():
    ...
    p.add_argument("--features", required=True,
                   help="Path to the .npz feature archive produced by "
                        "extraction/extract_hf.py (must contain a 'features' "
                        "[N, D] array and a 'filenames' [N] array).")
...
# ---------------------------------------------
# 4.  extraction/extract_hf.py .npz feature loading
# ---------------------------------------------

def load_npz_features(samples, features_path, filename_map=None):
    """
    Loads the single .npz archive produced by extraction/extract_hf.py and
    builds a RAM lookup dictionary {filename: feature_vector}, restricted to
    filenames that appear in `samples`. Matching is by exact filename
    (basename with extension), since that is exactly what extract_hf.py
    stores in 'filenames'.

    Every vector is expected to already be flat (1D) per image, as
    extract_hf.py only ever saves [N, D] arrays (D depends on --mode: cls vs
    cls_avg, or gap vs flatten for sam3). This function raises an error
    rather than pooling if that assumption is violated, so a
    malformed/incompatible .npz fails loudly instead of silently producing
    ...
    if "features" not in data or "filenames" not in data:
        raise ValueError(
            f"'{features_path}' does not look like an extract_hf.py output "
    ...
    if raw_features.ndim != 2:
        raise ValueError(
            ...
            f"flat per-image vectors produced by extraction/extract_hf.py."
        )
